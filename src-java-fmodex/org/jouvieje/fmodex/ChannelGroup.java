...
public class ChannelGroup extends Pointer {
	...
	public static ChannelGroup asChannelGroup(Pointer pointer) {
		...
		return new ChannelGroup(address);
	}
	...
	public FMOD_RESULT getMemoryInfo(int memorybits, int event_memorybits, IntBuffer memoryused, FMOD_MEMORY_USAGE_DETAILS memoryused_details) {
		...
		int javaResult = FmodExJNI.ChannelGroup_getMemoryInfo(pointer, memorybits, event_memorybits, memoryused, BufferUtils.getPositionInBytes(memoryused), Pointer.getPointer(memoryused_details));
		return FMOD_RESULT.get(javaResult);
	}
