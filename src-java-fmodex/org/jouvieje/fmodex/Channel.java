...
public class Channel extends Pointer {
	...
	public FMOD_RESULT setLowPassGain(float gain) {
		if(pointer == 0) throw new NullPointerException();
		int javaResult = FmodExJNI.Channel_setLowPassGain(pointer, gain);
		return FMOD_RESULT.get(javaResult);
	}
	 ...
	 * 
	 */
	public FMOD_RESULT getLowPassGain(FloatBuffer gain) {
		if(pointer == 0) throw new NullPointerException();
		if(gain != null && !gain.isDirect()) {
			throw new NonDirectBufferException();
		}
		int javaResult = FmodExJNI.Channel_getLowPassGain(pointer, gain, BufferUtils.getPositionInBytes(gain));
		return FMOD_RESULT.get(javaResult);
	}
	...
	public FMOD_RESULT setCallback(FMOD_CHANNEL_CALLBACK callback) {
		...
		int javaResult = FmodExJNI.Channel_setCallback(pointer, callback == null ? false : true);
		return FMOD_RESULT.get(javaResult);
	}
	 ...
	 * 
	 */
	public FMOD_RESULT isVirtual(ByteBuffer is) {
		if(pointer == 0) throw new NullPointerException();
		if(is != null && !is.isDirect()) {
			throw new NonDirectBufferException();
		}
		int javaResult = FmodExJNI.Channel_isVirtual(pointer, is, BufferUtils.getPositionInBytes(is));
		return FMOD_RESULT.get(javaResult);
	}
	...
	public FMOD_RESULT getMemoryInfo(int memorybits, int event_memorybits, IntBuffer memoryused, FMOD_MEMORY_USAGE_DETAILS memoryused_details) {
		...
		int javaResult = FmodExJNI.Channel_getMemoryInfo(pointer, memorybits, event_memorybits, memoryused, BufferUtils.getPositionInBytes(memoryused), Pointer.getPointer(memoryused_details));
		return FMOD_RESULT.get(javaResult);
	}
