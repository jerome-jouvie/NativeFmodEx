...
public class DSP extends Pointer {
	 ...
	 * 
	 */
	public FMOD_RESULT showConfigDialog(java.awt.Component hwnd, boolean show) {
		if(pointer == 0) throw new NullPointerException();
		long hwndHwnd = FmodExJNI.getHwnd(hwnd);
		if(hwndHwnd == 0) {
			hwndHwnd = hwnd.hashCode();
		}
		ComponentManager.addComponent(hwndHwnd, hwnd);

		int javaResult = FmodExJNI.DSP_showConfigDialog(pointer, hwnd, hwndHwnd, show);
		return FMOD_RESULT.get(javaResult);
	}
	...
	public FMOD_RESULT getMemoryInfo(int memorybits, int event_memorybits, IntBuffer memoryused, FMOD_MEMORY_USAGE_DETAILS memoryused_details) {
		...
		int javaResult = FmodExJNI.DSP_getMemoryInfo(pointer, memorybits, event_memorybits, memoryused, BufferUtils.getPositionInBytes(memoryused), Pointer.getPointer(memoryused_details));
		return FMOD_RESULT.get(javaResult);
	}
